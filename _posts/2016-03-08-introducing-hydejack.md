...
**NOTE**: 실제 설계시에 가장 중요한 사항들 OSI전체를 관통하는 핵심들에 대해서 다시 한번 정리해본다. 

{:.message}

## Availabillity 가용성
어떤 환경에서도 서비스가 정상적으로 유지될 수 있는지.. 이로 인해 IDC에 인프라를 설계했지만 IDC장애시에도 무용지물이 되었다. 카카오에 대한 장애회고 결국은 Multi-AZ 가용영역 Failover Auto Healing 자동복구

## Scalability 확장성 
Traffic 증가에 대한 대응.. 40G BW로 설계했는데 순간적인 폭증으로 100G 이상의 트래픽이 몰린적도 있고 C class 대역으로 IP설계가 충분하지 않은 상황도 발생했다. 결국에는 horizontal scaling /Stateless /Cache 

대부분 서비스의 규모가 점차 팽창하고 성장하다 보면 기술부채가 생기기 마련이다. 기술부채로 인한 딜레마 상황에 빠지게 된다. 수평적 확장을 할 수있는 설계가 미리되어 있다면 좋겠지만 결국에는 길고 Migration 작업을 거치게 된다. 
...
## Performance 성능 
최종적인 서비스 영역이던 내부구성요소이던지 간에 지속적인 성능을 발휘하려면 병목을 제거해야 한다. `Bottleneck`:
...
이외 추가적인 사항들은 운영성 모니터링 비용과 복구전략 유연성 자동화등에 대한 고려가 되어야 하지만 이상적인 항목이 되는 경우가 많다. 하지만 운영과 모니터링 DR 복구전략에 대해서는 명확한 대비를 해야한다. 
...
3.  비용 Cost
    어쩌면 가장 중요한 항목일지도 모른다.  Cloud 에서는 특히 더 
